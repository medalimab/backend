...
$(document).ready(function ($) {
    ...
    .on("click", function () {
      ...
      form.validate({
        submitHandler: function () {
          // 📍 First, fix the phone input
          const phoneInput = form.find('input[name="phone"]')[0];
          const iti = window.intlTelInputGlobals.getInstance(phoneInput);
          if (iti && iti.isValidNumber()) {
            phoneInput.value = iti.getNumber(); // ✅ Now full international number
          }

          // 📍 Then submit
          $.post(
              "assets/php/process_lead.php",
              form.serialize(),
              function (response) {
  ...
  $(".apartment-details").on("click", function () {
    const title = $(this).data("title"); // e.g., "1 Room Apartment"
    const price = $(this).data("price");
    const description = $(this).data("description");
    const parameters = $(this).data("parameters");

    // ✅ Inject apartment name into the Download Brochure button inside #floor-modal
    $(".open-lead-modal").attr("data-apartment", title);

    // Update modal content
    $("#floor-modal .modal-title").text(title);
    $("#floor-modal h2").text(price);
    $("#floor-modal .modal-body .right p").text(description);

    const dl = $("#floor-modal .modal-body .right dl");
    dl.empty();

    parameters.forEach((param) => {
      dl.append("<dt>" + param.label + "</dt>");
      dl.append("<dd>" + param.value + "</dd>");
    });
  });
